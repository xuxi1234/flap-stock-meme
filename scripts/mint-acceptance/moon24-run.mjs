...
import {Stop,requireThat,equal,ACCOUNT,TOKEN,DISTRIBUTOR,AMOUNT,TOTAL,CONFIRM,BATCHES,plan,artifact,erc20,callFor,completed,dueAt,next,validate,deliveries,persistThenBroadcast,batchId} from './moon24-core.mjs';
import {githubApi,openGitHubStore,REPOSITORY} from './moon24-store.mjs';
...
export function executionAllowed(env){
 return env.GITHUB_ACTIONS==='true'&&env.GITHUB_REPOSITORY===REPOSITORY&&env.GITHUB_REF==='refs/heads/main'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.MOON24_CONFIRM===CONFIRM;
}
function report(j){
 ...
 const text='蝴蝶中秋：已确认 '+completed(j)+'/113 批，'+ds.length+'/22533 地址。每地址转出1枚，实际到账按代币税费扣除，不补发。实际到账合计 '+formatEther(net)+' 枚；本任务Gas '+formatEther(gas)+' BNB。至少间隔900秒。\n';
 console.log(text);if(process.env.GITHUB_STEP_SUMMARY)fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY,text);
}
export async function main(env=process.env){
 const execute=env.MOON24_EXECUTE==='true';
 ...
 const api=githubApi(env.GITHUB_TOKEN),store=await openGitHubStore({api,readOnly:!execute,reportDirectory:env.MOON24_REPORT_DIR});
 ...
 console.log('固定22533名单、双节点、交易序号和批次状态通过；剩余需要 '+formatEther(remaining)+' 枚。');
 ...
 while(await chainNow()<dueAt(j)){requireThat(Date.now()<deadline,'等待到期超时。');console.log('等待至少15分钟间隔，目标UTC '+new Date(dueAt(j)*1000).toISOString());await sleep(15000);}
