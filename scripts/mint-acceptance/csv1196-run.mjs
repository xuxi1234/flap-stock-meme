import {csvInterludeRows} from './resume72-interlude.mjs';
import {prior72} from './csv1196-core.mjs';
import {baseline as previousBaseline,reconcile as reconcile72,verifyMappings as verifyMappings72} from './new72-run.mjs';
import {hydrate as hydrate72} from './new72-store.mjs';
...
import { ACCOUNT, TOKEN, DISTRIBUTOR, AMOUNT, TOTAL, artifact, erc20, history, plan, batchId, fresh, validate, next, callFor, persistThenBroadcast, requireThat, equal } from './csv1196-core.mjs';
import { accountBudget, reserveCampaign, rawTotal, OWNED_RETURN } from './new72-budget.mjs';

import { isDue, dueAt } from './csv1196-schedule.mjs';
export function verifyDelivery(e,logs){
 ...
 requireThat(delivered.length===plan()[e.batch].length&&completed.length===1,'成功回执缺少完整的 200 个地址发放事件。');
...
export async function baseline(clients){
 ...
 const base=[...await previousBaseline(clients),...await csvInterludeRows(clients)];
 const j=hydrate72(structuredClone(prior72));
 const checked=await reconcile72(clients,j,base,async()=>{},true);
 requireThat(j.entries.every(e=>e.settled&&e.success),'旧任务回执未全部核实。');
 await verifyMappings72(clients,j);
 return checked.rows;
}
...
async function verifyMappingsOnce(clients,j,only){
 await mapReads(only===undefined?Array.from({length:6},(_,i)=>i):[only],async i=>{
...
export async function run({clients,store,execute=false,ownedReturnConfirmed=false,accountProvider,reportDirectory}){
 ...
 console.log('钱包交易序号和6批链上完成状态核对通过。');
 ...
 if(completedCount===6){requireThat(state.allowance===0n,'任务完成后授权仍非零，停止核查。');j.active=false;await save(j);report(j,reportDirectory);return j;}
 ...
 requireThat(j.active&&j.authorization==='1196x1:6batches:budget0.1:lifetime','定时任务未由用户启动或已经暂停。');
 ...
 for(let step=0;step<4;step++){
  ...
  if(!expected){j.active=false;await save(j);report(j,reportDirectory);console.log('6 批已全部完成；不会新建第二组 1196 地址。');return j;}
  ...
  console.log(`已发送 ${entry.kind}${entry.batch===undefined?'':` 第 ${entry.batch+1}/6 轮`}：${hash}`);
  ...
  if(entry.kind==='send'){if(j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length===6){requireThat((await inspect(clients)).allowance===0n,'最后一轮后授权仍非零，停止并核查。');j.active=false;await save(j);}return j;}
 }
