...
const codeFixture = C.json(new URL('../../docs/vault-live-evidence/mint-user-state.json', import.meta.url));
const initial = () => ({ total: 0n, shares: 0n, claimed: 0n, launched: false, aborted: false, token: C.ACCOUNT, claimable: [0n, 0n], timestamp: 1789183300n });
const row = (nonce, value = 0n, success = true, fee = 100n) => ({ nonce, valueWei: String(value), success, feeWei: String(fee), events: [] });
const hex = n => '0x' + BigInt(n).toString(16);
function fakeClient(state = initial()) {
  ...
  f.readContract = async ({ functionName }) => ({ owner: C.ACCOUNT, commissionReceiver: C.ACCOUNT, vaultPortal: C.VAULT_PORTAL, implementation: C.IMPLEMENTATION, isCampaign: true, creator: C.ACCOUNT, factory: C.FACTORY, config: C.terms, totalShares: state.total, sharesOf: state.shares, claimedShares: state.claimed, launched: state.launched, aborted: state.aborted, token: state.token, claimable: state.claimable })[functionName];
  ...
  f.request = async ({ method, params: [hash] }) => {
    ...
    if (method === 'eth_getTransactionByHash') return { hash, from: r.from || C.ACCOUNT, to: r.to || C.CAMPAIGN, input: r.data || '0x', value: hex(r.valueWei), gas: hex(100n), gasPrice: hex(BigInt(r.feeWei) / 10n), chainId: '0x38', nonce: hex(r.nonce), blockHash: '0xabc' };
    return { transactionHash: hash, blockHash: '0xabc', blockNumber: hex(100), status: r.success ? '0x1' : '0x0', gasUsed: hex(10n), effectiveGasPrice: hex(BigInt(r.feeWei) / 10n), logs: [] };
  };
  return f;
}
