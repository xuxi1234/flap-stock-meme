import { BUDGET, ACCOUNT } from './holders18-core.mjs';
import { requireThat, equal, reserve } from './holders18-core.mjs';
...
export function accountBudget(rows,confirmed=false){
 requireThat(confirmed===true,'请在手动运行入口明确确认：仅将已核实的旧钱包转回22914钱包的本金单独记账，Gas仍计入累计0.2 BNB；新任务Gas不得超出累计0.2 BNB的剩余额度。未确认时不执行。');
...
function calculateBudget(rows){
 ...
 requireThat(spent<=BUDGET,'累计支出超过0.2 BNB，停止。');return spent;
}
