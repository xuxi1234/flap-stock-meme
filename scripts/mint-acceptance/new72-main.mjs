...
export async function assertOldStopped(api){
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-30x200-timed-ledger');
 requireThat(r.encoding==='base64'&&r.size<1000000,'旧任务检查点读取异常。');
 const old=JSON.parse(Buffer.from(r.content,'base64').toString('utf8'));
 const expected=JSON.parse(fs.readFileSync(new URL('./data/new72-cancelled-old.json',import.meta.url),'utf8'));
 requireThat(old.active===false&&JSON.stringify(old.entries)===JSON.stringify(expected.entries),'旧任务仍启用或交易记录变化，禁止新任务发送。');
}
export function options(env){
 const operation=env.NEW72_OPERATION||'check',execute=env.NEW72_PHASE==='execute';
