...
import { fresh as freshJournal, validate as validateJournal, callFor } from './holders18-core.mjs';
...
export const STATE_BRANCH = 'automation/airdrop-holders18-ledger';
  ...
  const save = async value => {
    ...
    if (!readOnly && previous !== text) {
      if (!sha) {
        ...
        for(let attempt=0;attempt<3;attempt++){
          try{
            const updated=await api('PUT',endpoint,{message:'Checkpoint holders18 execution',branch:STATE_BRANCH,sha,content:Buffer.from(text).toString('base64')});
