...
function eventLog(name, args) {
  const abi = C.campaignAbi.find(a => a.type === 'event' && a.name === name);
  const plain = abi.inputs.filter(i => !i.indexed);
  return { address: C.CAMPAIGN, topics: encodeEventTopics({ abi: [abi], eventName: name, args }), data: encodeAbiParameters(plain, plain.map(i => args[i.name])) };
}

export async function simulate({ loseBroadcastResponse = false } = {}) {
  const f = fakeClient(initial()), operations = [];
  C.CHECKPOINTS.forEach((h, n) => f.receiptRows.set(h, row(n, n === 3 ? C.SHARE_PRICE : 0n)));
  ...
  f.sendRawTransaction = async ({ serializedTransaction }) => {
    ...
    const decoded = decodeFunctionData({ abi: C.campaignAbi, data: t.data }); let log;
    ...
    f.receiptRows.set(hash, { ...row(t.nonce, BigInt(t.value), true, BigInt(t.gasPrice) * 80000n), to: t.to, data: t.data, gas: t.gas, gasPrice: t.gasPrice, gasUsed: 80000n, logs: [log] });
    f.latest++; f.pending = f.latest;
    if (loseBroadcastResponse && !simulatedLoss) { simulatedLoss = true; throw Error('simulated response lost after inclusion'); }
    return hash;
  };
  for (let n = 0; n < 5; n++) {
  ...
  requireThat(nextAction(await inspect(f)) === 'done' && operations.join(',') === 'mint,launch,claim', '模拟执行步骤缺失或发生重复。');
