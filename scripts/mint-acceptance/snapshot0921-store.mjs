...
import { fresh as freshJournal, validate as validateJournal, callFor } from './snapshot0921-core.mjs';
...
export const STATE_BRANCH = 'automation/airdrop-snapshot0921-ledger';
  ...
  const save = async value => {
    ...
    if (!readOnly && previous !== text) {
      if (!sha) {
        ...
        for(let attempt=0;attempt<3;attempt++){
          try{
            const updated=await api('PUT',endpoint,{message:'Checkpoint snapshot0921 execution',branch:STATE_BRANCH,sha,content:Buffer.from(text).toString('base64')});
