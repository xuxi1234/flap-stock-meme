import {BATCHES} from './image10-core.mjs';
import { pathToFileURL } from 'node:url';
import { Stop,requireThat } from './core.mjs';
import { dueAt } from './image10-schedule.mjs';
import { main,assertPrior72 } from './image10-main.mjs';
import { openGitHubStore,githubApi } from './image10-store.mjs';
...
export async function mainChain(env=process.env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.IMAGE10_CONFIRM==='image10x1:200:1200seconds:budget0.2:lifetime','串行续跑只允许用户在本仓库main手动确认后启动。');
 ...
 await runSlot({load:async()=>(await openGitHubStore({api,readOnly:true})).journal,execute:()=>main({...env,IMAGE10_OPERATION:'start',IMAGE10_PHASE:'execute'})});
}
