...
import {configure,batchId as holderBatch} from './image10-core.mjs';
...
import { plan,callFor,artifact,fresh,AMOUNT,TOTAL,history,validate,ACCOUNT,TOKEN,DISTRIBUTOR,erc20 } from './image10-core.mjs';
import { dueAt,isDue,authorize,eligible,shouldWake,recoveryFingerprint,readyForExecution } from './image10-schedule.mjs';
import fs from 'node:fs';
import { compact,hydrate } from './image10-store.mjs';
import { run } from './image10-run.mjs';
import { options } from './image10-main.mjs';
...
function networkFixture({taxed=false}={}){
 ...
 configure(source,sourceSha,PRIOR_HOLDERS);
 const starting=PRIOR71,doneBefore=0;
 const receipts=new Map(),txs=new Map(),signed=new Map(),completed=new Set();let now=1789463840;const times=new Map();let nonce=183,allowance=20000000000000000000n,balance=20000000000000000000000n,drop=true,dropAt=1,broadcasts=0,durable;
 ...
 for(const e of PRIOR_HOLDERS.entries){
  ...
  const logs=e.kind==='send'?holderOldPlan()[e.batch].map(recipient=>({address:DISTRIBUTOR,topics:encodeEventTopics({abi:artifact.abi,eventName:'Delivered',args}),data:encodeAbiParameters([{type:'address'},{type:'uint256'},{type:'uint256'}],[recipient,AMOUNT,970000000000000000n])})):[];
  if(e.kind==='send'){const count=BigInt(holderOldPlan()[e.batch].length);logs.push({address:DISTRIBUTOR,topics:encodeEventTopics({abi:artifact.abi,eventName:'BatchCompleted',args}),data:encodeAbiParameters([{type:'uint256'},{type:'uint256'}],[count,count*AMOUNT])});oldCompleted.add(args.batchId);}
  txs.set(e.hash,{hash:e.hash,from:ACCOUNT,chainId:'0x38',blockHash,nonce:toHex(t.nonce),to:call.to,input:call.data,value:'0x0',gas:toHex(BigInt(t.gas)),gasPrice:toHex(BigInt(t.gasPrice))});
  times.set(t.nonce+1000,e.confirmedAt);receipts.set(e.hash,{...receipt(e.hash,ACCOUNT,BigInt(e.feeWei)/BigInt(t.gasPrice),BigInt(t.gasPrice),logs),blockNumber:toHex(t.nonce+1000)});
 }
 const store=(readOnly=false)=>({journal:durable?hydrate(JSON.parse(durable)):{...fresh(),active:true,authorization:'image10x1:200:1200seconds:budget0.2:lifetime'},save:async j=>{validate(j);if(!readOnly)durable=JSON.stringify(compact(j));}});
 const client={
...
test('preserves all 71 old batches, accounts above 0.1, recovers lost reply and waits 1200 seconds',async t=>{
 ...
 assert.equal(checked.journal.spentWei,'131245270149141262');assert.equal(f.stats().broadcasts,0);
 ...
 f.setNow(1789465039);await run({...args,store:f.store()});assert.equal(f.stats().completed,1);
 f.setNow(1789465040);await run({...args,store:f.store()});assert.equal(f.stats().completed,2);
 f.setNow(1789466240);await run({...args,store:f.store()});assert.equal(f.stats().completed,3);
...
test('recovers a mined taxed batch without rebroadcast and continues after 1200 seconds',async t=>{
 t.mock.method(console,'log',()=>{});
