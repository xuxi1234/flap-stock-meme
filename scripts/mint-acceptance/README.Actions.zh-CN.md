...
你在 GitHub 设置页面保存专用钱包私钥，由你点击 **Run workflow** 启动。脚本自动完成本会话已确认的剩余认购、满额发射和领取，并逐笔保存哈希。你不需要在电脑上安装 Node.js，也不需要逐笔复制哈希。
...
| 名称 | 必填 | 值 |
| --- | --- | --- |
| `FLAP_MINT_PRIVATE_KEY` | 执行必填 | 地址 `0x79F8b832DE72e81Ad34fd66EcbbF673613264072` 对应的私钥，可带 `0x` 前缀 |
| `FLAP_BSC_RPC_URL` | 可选 | BSC 主网 HTTPS RPC；不填使用 `https://bsc-dataseed.bnbchain.org` |

**必须是本会话已确认的原验收钱包。** 若你添加另一个新钱包的私钥，脚本会因地址不符停止。不能把助记词填入此字段。私钥只填 GitHub Secrets 的值，不要放进代码、Issues、PR 评论或 Run workflow 输入框。
...
1. 认购 2 份，转入项目 **0.02 BNB + Gas**。
2. 满额发射：项目内部使用已存入的 0.02 BNB 创建代币和首购，钱包另付 Gas；最低输出 100,000 枚。
3. 向同一个固定钱包领取代币及合约可退余款，钱包另付 Gas。
...
| --- | --- |
| 链 | BSC 主网，56 |
| 签名钱包、管理员、平台佣金/领币接收人 | `0x79F8b832DE72e81Ad34fd66EcbbF673613264072` |
| Mint 工厂 | `0xA64186DB66bAed9fDC13678c4Cfb89B1e1E9cCa9` |
| Mint 实现 | `0x904C9E436299A65Fc25f32c48F3f908534F0e988` |
| 唯一直接交易目标 | `0xC7c1BcD4F0d25e04Ca3CA58139284B7E075B5eb1` |
...
累计费用从此钱包 nonce 0 开始计算：**成功转出的 BNB + 成功及失败交易实际 Gas**，发送前再加本笔金额和最大 Gas 预留。退款及后续入账不抵扣额度，不会每次 Run workflow 重置预算。发射内部使用的认购款已经计入，不重复加算本金。

预置此前 5 笔已确认交易（普通金库创建、Mint 工厂、项目、认购 1 份、退款）。先前只读核验计费累计为 `0.010548475729141262 BNB`，本次使用实时回执重算。钱包出现任何额外 nonce，包括失败交易，必须通过 `import_hashes` 补齐，否则停止。不要同时用本机脚本、页面或其他工作流操作此钱包。
...
尚未添加/读取你的钱包 Secret，尚未启动真实主网工作流；真实资金结果仍以你执行后的链上回执为准。测试通过不等于独立合约审计。Secret 可在验收完成后由你删除。
