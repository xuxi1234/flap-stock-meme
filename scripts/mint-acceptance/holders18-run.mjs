import {csv1196InterludeRows} from './resume72-csv1196.mjs';
import {csvInterludeRows} from './resume72-interlude.mjs';
import {prior72} from './holders18-core.mjs';
...
import { ACCOUNT, TOKEN, DISTRIBUTOR, AMOUNT, TOTAL, BATCHES, artifact, erc20, history, plan, batchId, fresh, validate, next, callFor, persistThenBroadcast, requireThat, equal } from './holders18-core.mjs';
import { accountBudget, reserveCampaign, rawTotal, OWNED_RETURN } from './holders18-budget.mjs';

import { isDue, dueAt } from './holders18-schedule.mjs';
...
export async function baseline(clients){
 ...
 const base=[...await previousBaseline(clients),...await csvInterludeRows(clients),...await csv1196InterludeRows(clients)];
...
export async function run({clients,store,execute=false,ownedReturnConfirmed=false,accountProvider,reportDirectory}){
 ...
 console.log(`钱包 ${ACCOUNT}；蝴蝶股票 ${formatEther(state.balance)} 枚；BNB ${formatEther(state.bnb)}；累计已用 ${formatEther(ledger.spent)} / 0.2 BNB。`);
 if(!execute){
 ...
 requireThat(j.active&&j.authorization==='holders18x1:200:1800seconds:budget0.2:lifetime','定时任务未由用户启动或已经暂停。');
