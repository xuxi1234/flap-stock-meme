import {prior72} from './image12-core.mjs';
import {baseline as previousBaseline,reconcile as reconcilePrevious,verifyMappings as verifyPrevious} from './image10-run.mjs';
import {hydrate as hydratePrevious} from './image10-store.mjs';
...
import { ACCOUNT, TOKEN, DISTRIBUTOR, AMOUNT, TOTAL, BATCHES, artifact, erc20, history, plan, batchId, fresh, validate, next, callFor, persistThenBroadcast, requireThat, equal } from './image12-core.mjs';
import { accountBudget, reserveCampaign, rawTotal, OWNED_RETURN } from './holders18-budget.mjs';

import { isDue, dueAt } from './image12-schedule.mjs';
export function verifyDelivery(e,logs){
 ...
 return delivered.map((l,i)=>{
  // User approved gross-0.1-token delivery: contract tax may
  // reduce net receipt. Keep identity, order and exact requested amount checks.
  const a=l.args;requireThat(equal(a.sender,ACCOUNT)&&equal(a.token,TOKEN)&&equal(a.batchId,batchId(e.batch))&&equal(a.recipient,plan()[e.batch][i])&&a.requested===AMOUNT,'逐地址发放事件与固定转出 0.1 枚的计划不符，已停止后续轮次。');
...
export async function run({clients,store,execute=false,ownedReturnConfirmed=false,accountProvider,reportDirectory}){
 ...
 requireThat(j.active&&j.authorization==='image12x0.1:200:1800seconds:budget0.2:lifetime','定时任务未由用户启动或已经暂停。');
