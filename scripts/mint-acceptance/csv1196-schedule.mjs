...
export const AUTHORIZATION='1196x1:6batches:budget0.1:lifetime';
export function dueAt(j){
 ...
 return last.confirmedAt+60;
}
export const isDue=(j,now)=>Number.isSafeInteger(now)&&now>=dueAt(j);
export function eligible(j,now){
 return j.active===true&&j.authorization===AUTHORIZATION&&j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length<6&&isDue(j,now);
}
export function authorize(j,env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.CSV1196_CONFIRM==='1196x1:6batches','只允许用户在本仓库main手动确认1196枚、6批和累计预算后启动。');
 requireThat(j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length<6,'本任务已完成，不能重新开始另一组空投。');
...
export function shouldWake(j,now){
 return j.active===true&&j.authorization===AUTHORIZATION&&(j.entries.some(e=>!e.settled)||j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length===6||isDue(j,now));
}
