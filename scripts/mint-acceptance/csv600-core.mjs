...
export const SOURCE_SHA='10c2b8ac16bd6ff591ae7c182fd986e95b9e6f5785bbbc87e336cae8d168df4d';
export const ID=keccak256(toHex(`butterfly:56:74a7:csv600:1:180seconds:${SOURCE_SHA}`));
...
export const BASE_NONCE=72;
let cached;
export function plan(){
 if(!cached){
  const bytes=readFileSync(new URL('./data/csv600-source.csv',import.meta.url));
  ...
  requireThat(addresses.length===600&&new Set(addresses).size===600&&addresses.every(a=>isAddress(a)&&![ACCOUNT,TOKEN,DISTRIBUTOR,'0x'+'0'.repeat(40)].some(b=>equal(a,b))),'名单数量、重复地址或格式无效。');
  cached=Object.freeze(Array.from({length:3},(_,i)=>Object.freeze(addresses.slice(i*200,(i+1)*200))));
 }return cached;
}
...
export function callFor(e){
 if(e.kind==='send'){
  requireThat(Number.isInteger(e.batch)&&e.batch>=0&&e.batch<3,'批次不在固定的 3 批范围内。');
...
export function validate(j){
 ...
 requireThat(typeof j.active==='boolean','定时任务开关无效。');
  requireThat(Array.isArray(j.entries)&&j.entries.length<=8,'执行记录结构异常。');
 let completed=0;
...
export function next(j,allowance){
 ...
 requireThat(done<=3,'批次超出上限。');
 const remaining=TOTAL-BigInt(done)*200n*AMOUNT;
 if(done===3)return allowance===0n?null:{kind:'reset',amount:'0'};
