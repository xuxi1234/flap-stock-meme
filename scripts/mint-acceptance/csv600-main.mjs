...
import { fresh } from './csv600-core.mjs';
import { run,report } from './csv600-run.mjs';
import { openGitHubStore,githubApi,REPOSITORY } from './csv600-store.mjs';
import { authorize,shouldWake,recoveryFingerprint,readyForExecution } from './csv600-schedule.mjs';
export async function assertPrior72(api){
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-72x200-ledger');
 requireThat(r.encoding==='base64'&&r.size<1000000,'旧72轮检查点读取异常。');
 const old=JSON.parse(Buffer.from(r.content,'base64').toString('utf8'));
 const expected=JSON.parse(fs.readFileSync(new URL('./data/csv600-prior72.json',import.meta.url),'utf8'));
 requireThat(old.id===expected.id&&isDeepStrictEqual(old.entries,expected.entries),'旧72轮已产生新交易；本任务固定基线不再适用，停止。');
 const run=await api('GET','/actions/runs/34751514715');
 requireThat(run.status==='completed'&&run.conclusion==='cancelled','旧72轮运行尚未取消，禁止并行空投。');
}
export function options(env){
 const operation=env.CSV600_OPERATION||'check',execute=env.CSV600_PHASE==='execute';
...
export async function main(env=process.env){
 ...
 if(env.GITHUB_TOKEN)await assertPrior72(githubApi(env.GITHUB_TOKEN));
 if(operation==='pause'){
