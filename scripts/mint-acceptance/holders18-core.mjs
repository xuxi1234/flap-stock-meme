...
export const BUDGET=200000000000000000n;
...
export function configure(source,sha,prior){
 requireThat(prior.version===3&&prior.entries.length>0&&prior.entries.every(e=>e.settled&&e.success&&Number.isSafeInteger(e.transaction?.nonce)),'旧任务仍有未确认或失败交易。');
...
export function reserve(spent,gas,price){
 ...
 requireThat(spent+fee<=BUDGET,'累计支出（含历史支出及 Gas）超过 0.2 BNB，停止。');return fee;
}
