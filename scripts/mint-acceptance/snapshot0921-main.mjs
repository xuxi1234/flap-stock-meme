import {assertPrior72 as configurePrevious} from './image12-main.mjs';
...
import { fresh,configure,BATCHES,plan } from './snapshot0921-core.mjs';
import { run,report } from './snapshot0921-run.mjs';
import { openGitHubStore,githubApi,REPOSITORY } from './snapshot0921-store.mjs';
import { authorize,shouldWake,recoveryFingerprint,readyForExecution } from './snapshot0921-schedule.mjs';
export async function assertPrior72(api){
 await configurePrevious(api);
 const prior=JSON.parse(fs.readFileSync(new URL('./data/snapshot0921-prior-image12.json',import.meta.url),'utf8'));
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-image12-ledger');
 requireThat(r.encoding==='base64'&&r.size<1000000,'旧持仓任务检查点读取异常。');
 const current=JSON.parse(Buffer.from(r.content,'base64').toString('utf8'));
 requireThat(current.version===prior.version&&current.id===prior.id&&current.active===false&&isDeepStrictEqual(current.entries,prior.entries),'旧持仓任务变化或仍在执行；停止核对预算和nonce。');
 requireThat(prior.active===false&&prior.entries.every(e=>e.settled&&e.success)&&prior.entries.filter(e=>e.kind==='send').length===36,'旧36批任务未完整结束。');
 const manifest=JSON.parse(fs.readFileSync(new URL('./data/snapshot0921-manifest.json',import.meta.url),'utf8'));
 requireThat(manifest.complete===true&&manifest.chainId===56&&manifest.tokenCount===10&&manifest.uniqueAddresses===5557&&manifest.rawTopCount===5633&&manifest.deduplicationOnly===true&&manifest.excludedAddresses===0,'固定5557地址名单尚未核验。');
 ...
 requireThat(plan().flat().length===5557&&BATCHES===28,'名单人数或批次数不匹配。');
}

export function options(env){
 const operation=env.SNAPSHOT0921_OPERATION||'check',execute=env.SNAPSHOT0921_PHASE==='execute';
 requireThat(['check','start','pause'].includes(operation),'定时操作无效。');
...
export async function main(env=process.env){
 ...
 if(env.GITHUB_OUTPUT)fs.appendFileSync(env.GITHUB_OUTPUT,`slots=${JSON.stringify(Array.from({length:BATCHES},(_,i)=>i+1))}\n`);
 ...
 const primary=env.FLAP_BSC_RPC_URL,secondary='https://bsc-dataseed1.defibit.io';
 requireThat(primary&&new URL(primary).protocol==='https:'&&new URL(primary).hostname==='bnb-mainnet.g.alchemy.com'&&primary!==secondary,'需要两个不同HTTPS节点。');
