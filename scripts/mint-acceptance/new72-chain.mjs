...
import { dueAt } from './new72-schedule.mjs';
import { main } from './new72-main.mjs';
import { openGitHubStore,githubApi } from './new72-store.mjs';
...
export async function runSlot({load,execute,now=()=>Math.floor(Date.now()/1000),sleep=ms=>new Promise(r=>setTimeout(r,ms)),log=console.log}){
 ...
 if(before>=72){log('72轮已完成，本排队步骤不再发币。');return;}
 ...
 while(now()<timeout){
  ...
  while(now()<target){
   ...
   const remaining=target-now();log(`已完成${completed(j)}/72轮，距下一轮至少${remaining}秒。`);
  ...
  j=await load();
  requireThat(!j.entries.some(e=>e.settled&&!e.success),'已有失败交易，保留记录并停止。');
  if(completed(j)>before||completed(j)>=72){log(`本步骤结束，已完成${completed(j)}/72轮。`);return;}
...
export async function mainChain(env=process.env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.NEW72_CONFIRM==='72x200x0.1','串行续跑只允许用户在本仓库main手动确认后启动。');
 const api=githubApi(env.GITHUB_TOKEN);

 await runSlot({load:async()=>(await openGitHubStore({api,readOnly:true})).journal,execute:()=>main({...env,NEW72_OPERATION:'start',NEW72_PHASE:'execute'})});
}
