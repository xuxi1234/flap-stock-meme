...
test('resume reconciles CSV costs, keeps27 rounds, recovers a lost reply and finishes72 without duplicate sends',async t=>{
 ...
 assert.equal(readonly.journal.spentWei,'72836583189141262');assert.equal(f.stats().broadcasts,0);
 await assert.rejects(()=>run({...args,store:f.store()}),/response was lost/);
 assert.equal(f.stats().completed,1);assert.equal(f.stats().broadcasts,2);
 await run({...args,store:f.store()});assert.equal(f.stats().broadcasts,2);
 f.setNow(1789341399);await run({...args,store:f.store()});assert.equal(f.stats().completed,1);
 let j;
 for(let i=1;i<45;i++){f.setNow(1789340200+i*1200);j=await run({...args,store:f.store()});assert.equal(f.stats().completed,i+1);}
 assert.equal(j.active,false);assert.equal(j.entries.filter(e=>e.kind==='send'&&e.received.length===200).length,72);
 assert.equal(f.stats().broadcasts,46);assert.equal(f.stats().nonce,123);assert.equal(f.stats().allowance,0n);
 assert.equal(BigInt(j.spentWei),72836583189141262n+230000000000000n);
 await run({...args,store:f.store()});assert.equal(f.stats().broadcasts,46);
});
test('unrecorded wallet transaction prevents signing',async t=>{
 t.mock.method(console,'log',()=>{});const f=networkFixture();
 f.client.getTransactionCount=async({address})=>address.toLowerCase()===ACCOUNT.toLowerCase()?78:6;
 await assert.rejects(()=>run({clients:[f.client,f.client],store:f.store(),execute:true,ownedReturnConfirmed:true,accountProvider:async()=>{assert.fail('must not sign')}}),/记录外交易/);
 assert.equal(f.stats().broadcasts,0);
});
test('fee spike stops before a signature or broadcast and retains27 rounds',async t=>{
