...
import { freshJournal, stringify, validateJournal, requireThat, Stop } from './core.mjs';

export const REPOSITORY = 'xuxi1234/flap-stock-meme';
export const STATE_BRANCH = 'automation/mint-acceptance-ledger';
...
export async function openGitHubStore({ api, readOnly = true, reportDirectory }) {
  ...
  if (existing) {
    ...
    journal = JSON.parse(previous); validateJournal(journal); sha = existing.sha;
  }
    ...
    if (!readOnly && previous !== text) {
      if (!sha) {
        ...
        const tree = await api('POST', '/git/trees', { tree: [
          ...
          { path: 'README.md', mode: '100644', type: 'blob', content: '# Mint execution checkpoints\nPublic transaction state only. No private keys or signed raw transactions. Do not reset this branch.\n' },
        ] });
        const commit = await api('POST', '/git/commits', { message: 'Initialize public Mint execution checkpoint', tree: tree.sha, parents: [] });
        ...
        const updated = await api('PUT', endpoint, { message: 'Checkpoint Mint execution before next network action', branch: STATE_BRANCH, sha, content: Buffer.from(text).toString('base64') });
