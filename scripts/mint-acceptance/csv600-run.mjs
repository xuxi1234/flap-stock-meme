...
import { ACCOUNT, TOKEN, DISTRIBUTOR, AMOUNT, TOTAL, artifact, erc20, history, plan, batchId, fresh, validate, next, callFor, persistThenBroadcast, requireThat, equal } from './csv600-core.mjs';
import { accountBudget, reserveCampaign, rawTotal, OWNED_RETURN } from './new72-budget.mjs';

import { isDue, dueAt } from './csv600-schedule.mjs';
...
export async function baseline(clients){
 ...
 const j=hydrate72(JSON.parse(fs.readFileSync(new URL('./data/csv600-prior72.json',import.meta.url),'utf8')));
 const checked=await reconcile72(clients,j,base,async()=>{},true);
 requireThat(j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length===27&&j.entries.every(e=>e.settled&&e.success),'旧任务27轮未全部核实，停止。');
...
async function verifyMappingsOnce(clients,j,only){
 await mapReads(only===undefined?Array.from({length:3},(_,i)=>i):[only],async i=>{
...
export async function run({clients,store,execute=false,ownedReturnConfirmed=false,accountProvider,reportDirectory}){
 ...
 await inspect(clients);console.log('双节点合约、余额及授权核对通过，正在核对历史预算及旧任务27轮交易。');const base=await baseline(clients);
 ...
 console.log('钱包交易序号和3批链上完成状态核对通过。');
 ...
 if(completedCount===3){requireThat(state.allowance===0n,'任务完成后授权仍非零，停止核查。');j.active=false;await save(j);report(j,reportDirectory);return j;}
 ...
 requireThat(j.active&&j.authorization==='3x200x1:budget0.1:lifetime','定时任务未由用户启动或已经暂停。');
 ...
 for(let step=0;step<4;step++){
  ...
  if(!expected){j.active=false;await save(j);report(j,reportDirectory);console.log('3 批已全部完成；不会新建第二组 600 地址。');return j;}
  ...
  console.log(`已发送 ${entry.kind}${entry.batch===undefined?'':` 第 ${entry.batch+1}/3 轮`}：${hash}`);
  ...
  if(entry.kind==='send'){if(j.entries.filter(e=>e.kind==='send'&&e.settled&&e.success).length===3){requireThat((await inspect(clients)).allowance===0n,'最后一轮后授权仍非零，停止并核查。');j.active=false;await save(j);}return j;}
 }
