...
import {configure,batchId as holderBatch} from './holders18-core.mjs';
...
import { plan,callFor,artifact,fresh,AMOUNT,TOTAL,history,validate,ACCOUNT,TOKEN,DISTRIBUTOR,erc20 } from './holders18-core.mjs';
import { dueAt,isDue,authorize,eligible,shouldWake,recoveryFingerprint,readyForExecution } from './holders18-schedule.mjs';
import fs from 'node:fs';
import { compact,hydrate } from './holders18-store.mjs';
import { run } from './holders18-run.mjs';
import { options } from './holders18-main.mjs';
...
function networkFixture({taxed=false}={}){
 ...
 const store=(readOnly=false)=>({journal:durable?hydrate(JSON.parse(durable)):{...fresh(),active:true,authorization:'holders18x1:200:1800seconds:budget0.2:lifetime'},save:async j=>{validate(j);if(!readOnly)durable=JSON.stringify(compact(j));}});
 const client={
...
test('preserves all 71 old batches, accounts above 0.1, recovers lost reply and waits 1800 seconds',async t=>{
 ...
 assert.equal(checked.journal.spentWei,'99777690609141262');assert.equal(f.stats().broadcasts,0);
