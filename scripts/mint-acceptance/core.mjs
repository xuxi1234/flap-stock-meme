...
export async function reconcile(client, journal, save) {
  ...
  journal.results = rows; journal.spentWei = spent.toString(); save(journal);
...
export async function prepare(client, state, ledger) {
  const action = nextAction(state); requireThat(action !== 'done', '验收已经完成。');
  const salt = action === 'launch' ? await mineSalt() : undefined;
...
export async function sendEntry({ client, account, entry, ledger, save, journal }) {
  ...
  entry.hash = hash; save(journal); // Write-ahead checkpoint MUST reach disk before network broadcast.
