import {assertPrior72 as configurePrevious} from './holders18-main.mjs';
...
import { fresh,configure,BATCHES,plan } from './image10-core.mjs';
import { run,report } from './image10-run.mjs';
import { openGitHubStore,githubApi,REPOSITORY } from './image10-store.mjs';
import { authorize,shouldWake,recoveryFingerprint,readyForExecution } from './image10-schedule.mjs';
export async function assertPrior72(api){
 await configurePrevious(api);
 const prior=JSON.parse(fs.readFileSync(new URL('./data/image10-prior-holders18.json',import.meta.url),'utf8'));
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-holders18-ledger');
 ...
 requireThat(prior.active===false&&prior.entries.every(e=>e.settled&&e.success)&&prior.entries.filter(e=>e.kind==='send').length===50,'旧50批任务未完整结束。');
 const manifest=JSON.parse(fs.readFileSync(new URL('./data/image10-manifest.json',import.meta.url),'utf8'));
 requireThat(manifest.complete===true&&manifest.chainId===56&&manifest.tokenCount===10&&manifest.uniqueAddresses===5774&&manifest.rawTopCount===6000&&manifest.deduplicationOnly===true&&manifest.excludedAddresses===0,'固定5774地址名单尚未核验。');
 ...
 requireThat(plan().flat().length===5774&&BATCHES===29,'名单人数或批次数不匹配。');
}

export function options(env){
 const operation=env.IMAGE10_OPERATION||'check',execute=env.IMAGE10_PHASE==='execute';
