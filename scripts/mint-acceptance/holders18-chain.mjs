import {BATCHES} from './holders18-core.mjs';
import { pathToFileURL } from 'node:url';
import { Stop,requireThat } from './core.mjs';
import { dueAt } from './holders18-schedule.mjs';
import { main,assertPrior72 } from './holders18-main.mjs';
import { openGitHubStore,githubApi } from './holders18-store.mjs';
...
export async function mainChain(env=process.env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.HOLDERS18_CONFIRM==='holders18x1:200:1800seconds:budget0.2:lifetime','串行续跑只允许用户在本仓库main手动确认后启动。');
 ...
 await runSlot({load:async()=>(await openGitHubStore({api,readOnly:true})).journal,execute:()=>main({...env,HOLDERS18_OPERATION:'start',HOLDERS18_PHASE:'execute'})});
}
