...
import { plan as previousTimedPlan } from './timed-airdrop-core.mjs';
...
export const TOTAL=1440000000000000000000n;
// Public campaign namespace; this is NOT a private key. Never change for a retry.
export const ID=keccak256(toHex('butterfly:56:74a7:2026-09-13:72x200:0.1:20minutes:campaign-2'));
...
export const BASE_NONCE=43;
let cached;
export function plan(){
 if(!cached){
  const addresses=Array.from({length:14400},(_,i)=>'0x'+keccak256(toHex(`${ID}:recipient:${i}`)).slice(-40));
  requireThat(new Set(addresses).size===14400&&addresses.every(a=>![ACCOUNT,TOKEN,DISTRIBUTOR,'0x'+'0'.repeat(40)].some(b=>equal(a,b))),'名单存在重复或非法接收地址。');
  const priorRecipients=new Set([...previousPlan().flat(),...previousTimedPlan().flat()]);
  requireThat(addresses.every(a=>!priorRecipients.has(a)),'新名单与上一任务重复。');
  cached=Object.freeze(Array.from({length:72},(_,i)=>Object.freeze(addresses.slice(i*200,(i+1)*200))));
 }return cached;
}
...
export function callFor(e){
 if(e.kind==='send'){
  requireThat(Number.isInteger(e.batch)&&e.batch>=0&&e.batch<72,'批次不在固定的 72 轮范围内。');
...
export function next(j,allowance){
 ...
 requireThat(done<=72,'批次超出上限。');
 const remaining=TOTAL-BigInt(done)*200n*AMOUNT;
 if(done===72)return allowance===0n?null:{kind:'reset',amount:'0'};
