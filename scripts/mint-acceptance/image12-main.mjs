import {assertPrior72 as configurePrevious} from './image10-main.mjs';
...
import { fresh,configure,BATCHES,plan } from './image12-core.mjs';
import { run,report } from './image12-run.mjs';
import { openGitHubStore,githubApi,REPOSITORY } from './image12-store.mjs';
import { authorize,shouldWake,recoveryFingerprint,readyForExecution } from './image12-schedule.mjs';
export async function assertPrior72(api){
 await configurePrevious(api);
 const prior=JSON.parse(fs.readFileSync(new URL('./data/image12-prior-image10.json',import.meta.url),'utf8'));
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-image10-ledger');
 ...
 requireThat(prior.active===false&&prior.entries.every(e=>e.settled&&e.success)&&prior.entries.filter(e=>e.kind==='send').length===29,'旧29批任务未完整结束。');
 const manifest=JSON.parse(fs.readFileSync(new URL('./data/image12-manifest.json',import.meta.url),'utf8'));
 requireThat(manifest.complete===true&&manifest.chainId===56&&manifest.tokenCount===12&&manifest.uniqueAddresses===7018&&manifest.rawTopCount===7200&&manifest.deduplicationOnly===true&&manifest.excludedAddresses===0,'固定7018地址名单尚未核验。');
 ...
 requireThat(plan().flat().length===7018&&BATCHES===36,'名单人数或批次数不匹配。');
}

export function options(env){
 const operation=env.IMAGE12_OPERATION||'check',execute=env.IMAGE12_PHASE==='execute';
