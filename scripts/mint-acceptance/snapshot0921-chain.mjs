import {BATCHES} from './snapshot0921-core.mjs';
import { pathToFileURL } from 'node:url';
import { Stop,requireThat } from './core.mjs';
import { dueAt } from './snapshot0921-schedule.mjs';
import { main,assertPrior72 } from './snapshot0921-main.mjs';
import { openGitHubStore,githubApi } from './snapshot0921-store.mjs';
...
export async function mainChain(env=process.env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.SNAPSHOT0921_CONFIRM==='snapshot0921x0.7777:200:3600seconds:budget0.3:lifetime','串行续跑只允许用户在本仓库main手动确认后启动。');
 ...
 await runSlot({load:async()=>(await openGitHubStore({api,readOnly:true})).journal,execute:()=>main({...env,SNAPSHOT0921_OPERATION:'start',SNAPSHOT0921_PHASE:'execute'})});
}
