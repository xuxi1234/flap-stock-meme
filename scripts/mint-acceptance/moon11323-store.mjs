...
import { stringify, requireThat, Stop } from './moon11323-core.mjs';
import { fresh as freshJournal, validate as validateJournal, callFor } from './moon11323-core.mjs';
...
export const STATE_BRANCH = 'automation/airdrop-moon11323-ledger';
  ...
  const save = async value => {
    ...
    if (!readOnly && previous !== text) {
      if (!sha) {
        ...
        for(let attempt=0;attempt<3;attempt++){
          try{
            const updated=await api('PUT',endpoint,{message:'Checkpoint moon11323 execution',branch:STATE_BRANCH,sha,content:Buffer.from(text).toString('base64')});
