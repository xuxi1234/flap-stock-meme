import { requireThat, BATCHES } from './image10-core.mjs';
export const AUTHORIZATION='image10x1:200:1200seconds:budget0.2:lifetime';
...
export function authorize(j,env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.IMAGE10_CONFIRM==='image10x1:200:1200seconds:budget0.2:lifetime','只允许用户在本仓库main手动确认固定名单、每批200地址、每20分钟一批和0.2 BNB累计预算后启动。');
