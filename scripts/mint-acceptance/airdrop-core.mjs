...
export const AMOUNT=7000000000000000000n;
export const TOTAL=28000000000000000000000n;
// Public campaign namespace; this is NOT a private key. Never change for a retry.
export const ID=keccak256(toHex('butterfly:56:74a7:2026-09-12:20x200:7:campaign-1'));
...
export const history=JSON.parse(readFileSync(new URL('./data/airdrop-history.json',import.meta.url),'utf8'));
export const BASE_NONCE=5;
let cached;
export function plan(){
 if(!cached){
  const addresses=Array.from({length:4000},(_,i)=>'0x'+keccak256(toHex(`${ID}:recipient:${i}`)).slice(-40));
  requireThat(new Set(addresses).size===4000&&addresses.every(a=>![ACCOUNT,TOKEN,DISTRIBUTOR,'0x'+'0'.repeat(40)].some(b=>equal(a,b))),'名单存在重复或非法接收地址。');
  cached=Object.freeze(Array.from({length:20},(_,i)=>Object.freeze(addresses.slice(i*200,(i+1)*200))));
 }return cached;
}
export const batchId=i=>keccak256(encodeAbiParameters([{type:'bytes32'},{type:'uint256'}],[ID,BigInt(i)]));
export const fresh=()=>({version:1,id:ID,account:ACCOUNT,token:TOKEN,distributor:DISTRIBUTOR,entries:[]});
export function callFor(e){
 if(e.kind==='send'){
  requireThat(Number.isInteger(e.batch)&&e.batch>=0&&e.batch<20,'批次不在固定的 20 轮范围内。');
...
export function next(j,allowance){
 ...
 requireThat(done<=20,'批次超出上限。');
 const remaining=TOTAL-BigInt(done)*200n*AMOUNT;
 if(done===20)return allowance===0n?null:{kind:'reset',amount:'0'};
