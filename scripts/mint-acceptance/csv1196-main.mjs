...
import { fresh,setPrior } from './csv1196-core.mjs';
import { run,report } from './csv1196-run.mjs';
import { openGitHubStore,githubApi,REPOSITORY } from './csv1196-store.mjs';
import { authorize,shouldWake,recoveryFingerprint,readyForExecution } from './csv1196-schedule.mjs';
export async function assertPrior72(api){
 ...
 setPrior(JSON.parse(Buffer.from(r.content,'base64').toString('utf8')));
}
export function options(env){
 const operation=env.CSV1196_OPERATION||'check',execute=env.CSV1196_PHASE==='execute';
