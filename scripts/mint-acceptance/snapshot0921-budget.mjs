import { BUDGET, ACCOUNT } from './snapshot0921-core.mjs';
import { requireThat, equal, reserve } from './snapshot0921-core.mjs';
...
export const CAMPAIGN_GAS_CAP=BUDGET-CHARGED_BASELINE;
...
export function accountBudget(rows,confirmed=false){
 requireThat(confirmed===true,'请在手动运行入口明确确认：仅将已核实的旧钱包转回22914钱包的本金单独记账，Gas仍计入累计0.3 BNB；新任务Gas不得超出累计0.3 BNB的剩余额度。未确认时不执行。');
 ...
 requireThat(campaignGas>=0n&&campaignGas<=CAMPAIGN_GAS_CAP,'本次空投累计Gas超出累计剩余额度，停止；恢复任务不会重置额度。');
...
export function reserveCampaign(spent,gas,price){
 ...
 requireThat(spent>=CHARGED_BASELINE&&spent-CHARGED_BASELINE+fee<=CAMPAIGN_GAS_CAP,'本次空投累计Gas加本笔最大Gas超过累计剩余额度，停止。');
 return fee;
}
...
function calculateBudget(rows){
 ...
 requireThat(spent<=BUDGET,'累计支出超过0.3 BNB，停止。');return spent;
}
