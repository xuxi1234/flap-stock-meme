...
export const SOURCE_SHA='25d7f8a3116accea168a1e26cdaa2af7cfd9c93bcd8116196dbf36e4efad7e64';
export const AMOUNT=10n**18n,INTERVAL=900,BATCHES=113,TOTAL=22533n*AMOUNT;
export const CONFIRM='moon24:1:200:900seconds';
const source=fs.readFileSync(new URL('./data/moon24-recipients.txt',import.meta.url));
...
requireThat(addresses.length===22533&&new Set(addresses.map(a=>a.toLowerCase())).size===22533&&addresses.every(a=>isAddress(a)),'固定名单不符。');
requireThat(addresses.every(a=>![ACCOUNT,TOKEN,DISTRIBUTOR,'0x'+'0'.repeat(40),'0x000000000000000000000000000000000000dead'].some(b=>equal(a,b))),'名单包含发送钱包、零地址或相关合约。');
export const plan=Array.from({length:BATCHES},(_,i)=>addresses.slice(i*200,i*200+200));
export const ID=keccak256(toHex('moon24:56:'+TOKEN+':'+ACCOUNT+':'+SOURCE_SHA+':1:900:v1'));
