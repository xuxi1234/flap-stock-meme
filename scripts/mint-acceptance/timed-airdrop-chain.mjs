...
import { dueAt } from './timed-airdrop-schedule.mjs';
import { main } from './timed-airdrop-main.mjs';
import { openGitHubStore,githubApi } from './timed-airdrop-store.mjs';
...
export async function runSlot({load,execute,now=()=>Math.floor(Date.now()/1000),sleep=ms=>new Promise(r=>setTimeout(r,ms)),log=console.log}){
 ...
 if(before>=30){log('30轮已完成，本排队步骤不再发币。');return;}
 requireThat(!j.entries.some(e=>e.settled&&!e.success),'已有失败交易，保留记录并停止。');
 const timeout=now()+35*60;
 while(now()<timeout){
  const target=dueAt(j);
  while(now()<target){
   requireThat(target<timeout,'等待时间异常，停止而不提前发币。');
   const remaining=target-now();log(`已完成${completed(j)}/30轮，距下一轮至少${remaining}秒。`);
...
export async function mainChain(env=process.env){
 requireThat(env.GITHUB_ACTIONS==='true'&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REPOSITORY==='xuxi1234/flap-stock-meme'&&env.GITHUB_REF==='refs/heads/main'&&env.TIMED_CONFIRM==='30x200x0.1','串行续跑只允许用户在本仓库main手动确认后启动。');
 const api=githubApi(env.GITHUB_TOKEN);
 const initial=(await openGitHubStore({api,readOnly:true})).journal;
 requireThat(completed(initial)>=1,'此入口仅续跑已确认的原任务，不能创建新的30轮任务。');
 await runSlot({load:async()=>(await openGitHubStore({api,readOnly:true})).journal,execute:()=>main({...env,TIMED_OPERATION:'start',TIMED_PHASE:'execute'})});
}
