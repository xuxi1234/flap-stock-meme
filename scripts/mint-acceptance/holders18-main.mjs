...
export async function assertPrior72(api){
 const prior=JSON.parse(fs.readFileSync(new URL('./data/holders18-prior71.json',import.meta.url),'utf8'));
 const r=await api('GET','/contents/journal.json?ref=automation%2Fairdrop-72x200-ledger');
 requireThat(r.encoding==='base64'&&r.size<1000000,'旧任务检查点读取异常。');
