...
import { fresh as freshJournal, validate as validateJournal, callFor } from './csv1196-core.mjs';
...
export const STATE_BRANCH = 'automation/airdrop-csv1196-ledger';
  ...
  const save = async value => {
    ...
    if (!readOnly && previous !== text) {
      if (!sha) {
        ...
        for(let attempt=0;attempt<3;attempt++){
          try{
            const updated=await api('PUT',endpoint,{message:'Checkpoint csv1196 execution',branch:STATE_BRANCH,sha,content:Buffer.from(text).toString('base64')});
