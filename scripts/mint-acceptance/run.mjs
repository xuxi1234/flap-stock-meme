...
export async function complete(client, store, state) {
...
export async function runAcceptance({ client, store, opts, accountProvider = confirmLocally }) {
  const { journal, save } = store;
    ...
    for (let count = 0; count < 5; count++) {
      ...
      if (!account) account = await accountProvider();
      const entry = pending || await prepare(client, state, ledger);
      if (!pending) { journal.entries.push(entry); await save(journal); }
...
async function main() {
  ...
  try { await runAcceptance({ client, store, opts }); }
