...
for(let index=0;index<SUPPLY;index++){
 ...
 assert.equal(metadata.properties.edition,x.id);assert.equal(metadata.properties.artwork_sha256,x.dna);assert.equal(metadata.properties.image_sha256,imageHash);assert(/^https?:\/\//.test(metadata.image));assert(metadata.image.endsWith(`/art/${x.id}.jpg`));assert.equal(metadata.attributes.length,5);
}
