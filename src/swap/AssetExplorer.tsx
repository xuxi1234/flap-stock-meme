...
export function AssetExplorer({ onSelect, favorites, onFavorite, snapshot, loading, error, onRefresh, now, storageError }: Props) {
  ...
  return <section id="swap-discover" className="swap-explorer" aria-label="资产发现">
    ...
    <div className="swap-explorer-source"><p>链上参考价来自 DEX Screener 收录的多平台参考池，优先取返回结果中流动性较高的 USDT / USDC / WBNB 池。涨跌、成交额和流动性均为该池数据；不等于美股行情或实际兑换报价。</p><a href="/butterfly-stock-list.json" target="_blank" rel="noopener noreferrer">公开代币列表 ↗</a></div>
  </section>
}
...
export function MarketDetail({ token, market, stale, onCopy }: { token: SwapToken; market?: TokenMarket; stale: boolean; onCopy: (text: string) => void }) {
  if (!token.stockSymbol && token.category !== 'community') return null
  return <details className="swap-asset-info"><summary><TokenIcon token={token} /><span>{token.name.split(' / ')[0]}<small>{token.stockSymbol ?? 'FLAP STOCK'} · 已收录</small></span><b>资产详情 ⌄</b></summary><div className="swap-asset-info-body"><div className="swap-info-market"><span>链上参考价<strong>{usd(market?.priceUsd)}</strong></span><span>参考池 24H 涨跌<strong className={(market?.change24h ?? 0) >= 0 ? 'swap-positive' : 'swap-negative'}>{percent(market?.change24h)}</strong></span></div>{stale && <p className="swap-small-note">参考行情已过期，请刷新资产看板。</p>}<p>{token.issuer}</p><div className="swap-contract-copy"><code>{token.address}</code><button aria-label={`复制 ${token.symbol} 合约`} onClick={() => onCopy(token.address)}>复制</button></div><div className="swap-info-links"><a href={`https://bscscan.com/token/${token.address}`} target="_blank" rel="noopener noreferrer">链上记录 ↗</a><a href={token.source} target="_blank" rel="noopener noreferrer">发行资料 ↗</a>{market && <a href={`https://dexscreener.com/bsc/${market.pairAddress}`} target="_blank" rel="noopener noreferrer">{market.protocol} 参考池 ↗</a>}</div><div className="swap-pool-list">{market?.pools?.map(pool => <a key={pool.pairAddress} href={`https://dexscreener.com/bsc/${pool.pairAddress}`} target="_blank" rel="noopener noreferrer"><span>{pool.dex} · {pool.protocol}<small>{pool.quoteSymbol} · {pool.dex === 'PancakeSwap' && ['V2','V3'].includes(pool.protocol) ? '可参与站内路径查询' : '外部参考池'}</small></span><b>{usd(pool.liquidityUsd,true)} ↗</b></a>)}</div><p className="swap-small-note">站内报价支持 PancakeSwap V2 / V3；其他平台、Infinity 仅展示参考池。参考价格不等于本次成交价，收录不代表安全担保。</p></div></details>
}
