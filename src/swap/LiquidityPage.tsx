...
export function LiquidityPage({client,wallet,account,onConnect,onBusy}:Props) {
  ...
  const [pairedAmount,setPairedAmount]=useState<bigint|null>(null)
  ...
  if(value&&pool&&pool.reserveA>0n&&pool.reserveB>0n)try {
    ...
    amountA=side==='a'?amount:(pairedAmount??amount*pool.reserveA/pool.reserveB)
    amountB=side==='b'?amount:(pairedAmount??amount*pool.reserveB/pool.reserveA)
  ...
  const select=(token:SwapToken)=>{if(picker==='a')setA(token);else setB(token);setValue('');setPairedAmount(null);setPicker(null);setMessage('')}
  const enter=(which:'a'|'b',text:string)=>{
    setSide(which);setValue(text);setPairedAmount(null)
    if(pool&&pool.reserveA>0n&&pool.reserveB>0n)try{const parsed=parseAmount(text,which==='a'?a.decimals:b.decimals);setPairedAmount(which==='a'?parsed*pool.reserveB/pool.reserveA:parsed*pool.reserveA/pool.reserveB)}catch{/* Validation is shown alongside the input. */}
  }
  const part=(which:'a'|'b',n:number)=>{
    ...
    enter(which,formatUnits(available*BigInt(n)/100n,t.decimals))
  }
  const openReview=async()=>{
    ...
    try{
      ...
      setPool(fresh)
      // Preserve both user-visible maximum amounts across approvals and refreshes.
      // The router may use less when the current pool ratio changes, never more.
      const desiredA=mode==='add'?amountA:0n,desiredB=mode==='add'?amountB:0n
      if(mode==='add')setPairedAmount(side==='a'?amountB:amountA)
