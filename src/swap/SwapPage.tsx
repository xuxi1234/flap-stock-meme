...
import { ROUTER, TOKENS, STOCK_TOKENS, MAG7_SYMBOLS, matchesToken, selectedStockToken, homeHref, tokenKey, type SwapToken } from './config'
...
export function SwapPage() {
  ...
  const [output, setOutput] = useState<SwapToken>(() => selectedStockToken(window.location.search) ?? TOKENS[1])
  ...
  const [tokenCategory, setTokenCategory] = useState<'all' | 'stocks' | 'mag7'>('all')
  ...
  const filteredTokens = [...TOKENS, ...customTokens].filter(t => matchesToken(t, search) && (tokenCategory === 'all' || (tokenCategory === 'mag7' ? MAG7_SYMBOLS.includes(t.stockSymbol ?? '') : !!t.stockSymbol)))
  ...
  return <div className="swap-page">
    ...
    <main className="swap-main">
        ...
        <section className="swap-workspace" id="swap-form" aria-label="代币兑换">
          <div className="swap-card">
            ...
            {activeTab === 'swap' ? <>
              ...
              {output.stockSymbol && <div className="swap-stock-details"><strong>{output.stockSymbol} · 已收录</strong><span>{output.issuer}</span><a href={`https://bscscan.com/token/${output.address}`} target="_blank" rel="noopener noreferrer">{output.address} ↗</a><a href={output.source} target="_blank" rel="noopener noreferrer">查看合约资料来源 ↗</a><small>链上兑换报价与美股行情价格分别计算。可用路径将在输入数量后检查。</small></div>}
      ...
      <section className="swap-stock-directory" aria-label="美股代币目录"><div className="swap-directory-head"><div><p className="swap-eyebrow">ON-CHAIN STOCKS / 02</p><h2>熟悉的公司，链上找到。</h2><p>{STOCK_TOKENS.length} 项美股与 ETF 代币 · 美股七姐妹已齐全</p></div><button onClick={() => { setTokenCategory('stocks'); setTokenSide('output'); setSearch('') }}>搜索美股代币 ↗</button></div><div className="swap-stock-grid">{STOCK_TOKENS.map(t => <button key={t.address} onClick={() => { setOutput(t); if (tokenKey(input) === tokenKey(t)) setInput(TOKENS[0]); setActiveTab('swap'); document.getElementById('swap-form')?.scrollIntoView({ behavior: 'smooth' }) }}><TokenIcon token={t} /><span><strong>{t.name.split(' / ')[0]}</strong><small>{t.stockSymbol} · {t.symbol}</small></span><span>↗</span></button>)}</div><p className="swap-small-note">已收录表示合约资料已核对，不代表安全担保。链上代币不等同于直接持有股票；实际可兑换性以当前 V2 报价为准。</p></section>
