...
import { ROUTER, TOKENS, STOCK_TOKENS, MAG7_SYMBOLS, matchesToken, selectedPair, sharePairUrl, homeHref, tokenKey, type SwapToken } from './config'
...
export function SwapPage() {
  ...
  return <div className="swap-page">
    ...
    <header className="swap-header">
      ...
      <div className="swap-header-actions"><button className="swap-invite-button" onClick={()=>setInviteOpen(true)}>邀请赚返佣 ↗</button><span className="swap-chain"><i /> BNB Chain</span><button className="swap-primary" onClick={() => setWalletOpen(true)}>{account ? short(account) : '连接钱包'}</button></div>
    </header>
    ...
    <footer className="swap-footer"><a href={home}>蝴蝶股票 <b>FLAP STOCK</b></a><p>流动性来源 PancakeSwap V2 / V3 · 服务费 0.8% · 邀请返佣 70% · 池费、代币税与网络费另计</p><a href={`https://bscscan.com/address/${feeDeployment() ?? ROUTER}`} target="_blank" rel="noopener noreferrer">查看路由合约 ↗</a></footer>
