...
import { ROUTER, TOKENS, BUTTERFLY, STOCK_TOKENS, MAG7_SYMBOLS, matchesToken, selectedPair, sharePairUrl, homeHref, tokenKey, type SwapToken } from './config'
import { quoteRouter, approveExact, displayAmount, executeSwap, friendlySwapError, getAllowance, getQuote, importToken, makeSwapClient, minimumReceived, parseAmount, readBalance, type SwapQuote, type SwapReview } from './service'
...
export function SwapPage() {
  ...
  const [input, setInput] = useState<SwapToken>(initialPair.input)
  const [output, setOutput] = useState<SwapToken>(initialPair.output)
  const [amount, setAmount] = useState('')
  const [manualSlippage, setManualSlippage] = useState<number | null>(null)
  // Asset policy, keyed by contract rather than symbol. Quote tax and price impact
  // are already accounted for separately; neither is added to this tolerance.
  const automaticSlippage = [input, output].some(token => token.address.toLowerCase() === BUTTERFLY.address.toLowerCase()) ? 400 : 50
  const slippage = manualSlippage ?? automaticSlippage
  ...
  const [tokenCategory, setTokenCategory] = useState<'all' | 'stocks' | 'mag7' | 'favorites'>('all')
  ...
  useEffect(() => {
    ...
    if (TOKENS.some(t => !t.native && t.address.toLowerCase() === search.trim().toLowerCase())) return
  ...
  const transact = async (approval: boolean) => {
    ...
    try {
      ...
      setTransactions(list => [{ hash: hash!, account: review.account, title, inputKey: tokenKey(review.quote.input), outputKey: approval ? undefined : tokenKey(review.quote.output), time: Date.now(), status: 'pending' as const }, ...list].slice(0, 12))
  ...
  const filteredTokens = [...TOKENS, ...customTokens].filter(t => matchesToken(t, search) && (tokenCategory === 'all' || (tokenCategory === 'mag7' ? MAG7_SYMBOLS.includes(t.stockSymbol ?? '') : tokenCategory === 'favorites' ? favorites.includes(t.address.toLowerCase()) : !!t.stockSymbol)))
  ...
  return <div className="swap-page">
      ...
      <div className="swap-layout">
        ...
        <section className="swap-workspace" id="swap-form" aria-label="代币兑换">
          <div className="swap-card">
            ...
            {shareLink && <label className="swap-share-field">兑换对链接 · 不含钱包和金额<input aria-label="兑换对分享链接" readOnly value={shareLink} onFocus={e => e.currentTarget.select()} /></label>}
            {activeTab === 'swap' ? <>
              ...
              <div className="swap-slippage-row"><span>滑点上限</span><button onClick={() => setSettings(true)}>{manualSlippage === null ? '自动 · ' : ''}{(slippage / 100).toFixed(1)}% <span>调整</span></button></div>
          ...
          <div className="swap-route-card"><div><span className="swap-route-symbol">↳</span><strong>{quote?.wrap ? 'BNB / WBNB 包装转换' : '你的兑换路径'}</strong><span className="swap-route-status">{quoteBusy ? '报价中' : quote ? stale ? '待刷新' : '已报价' : '等待数量'}</span></div>{quote ? <><div className="swap-path-tokens">{quote.path.map((a, i) => { const token = pathToken(a); return <span key={a}>{i > 0 && <i>→</i>}{token && <TokenIcon token={token} />}<b>{token?.symbol ?? short(a)}</b></span> })}</div><small>区块 #{quote.block.toString()} · {quote.wrap ? '1:1 转换' : `PancakeSwap ${quote.protocol ?? 'V2'} · ${quote.path.length - 1} 次池内兑换`}</small>{!quote.wrap && <details className="swap-route-comparison"><summary>已比较 {quote.checkedPaths} 条候选路径 · {quote.alternatives?.length} 条有报价</summary><div>{quote.alternatives?.map((r, i) => <div key={`${r.protocol}-${r.path.join('-')}-${r.fees?.join('-')}`}><span>{r.protocol ?? 'V2'} · {r.path.map(a => pathToken(a)?.symbol ?? short(a)).join(' → ')}{i === 0 && <em>最高输出</em>}</span><b>{displayAmount(r.amountOut, output.decimals)} {output.symbol}</b></div>)}</div><p>相同区块比较 V2（最多3池）与 V3（最多2池、四档费率）。按预计到账排序，未扣网络费；不含跨协议混合路径、Infinity及其他交易所。</p></details>}</> : <p>输入数量后比较 V2 / V3 直连及常用中间币路径；蝴蝶股票使用支持代币税的 V2。</p>}</div>
        </section>
      </div>
    ...
    {settings && <Dialog title="兑换设置" onClose={() => setSettings(false)}><h3>滑点上限</h3><p className="swap-dialog-description">允许报价与实际成交之间的最大差异。</p><div className="swap-settings-options"><button aria-pressed={manualSlippage === null} onClick={() => { setManualSlippage(null); setCustomSlippage('') }}>自动</button>{[10, 50, 100, 400].map(n => <button key={n} aria-pressed={manualSlippage === n} onClick={() => { setManualSlippage(n); setCustomSlippage('') }}>{n / 100}%</button>)}<label><input aria-label="自定义滑点百分比" placeholder="自定义" inputMode="decimal" value={customSlippage} onChange={e => { setCustomSlippage(e.target.value); const n = Number(e.target.value); if (/^\d*\.?\d{0,1}$/.test(e.target.value) && n >= .1 && n <= 5) setManualSlippage(Math.round(n * 100)) }} />%</label></div><p className="swap-dialog-description">当前生效：{slippage / 100}% · 支持 0.1%–5%</p>{manualSlippage === null && <p className="swap-dialog-description">自动规则：蝴蝶股票交易 4%，其他兑换 0.5%，随所选代币切换。BNB / WBNB 包装按 1:1 执行。已知代币税已计入报价；未收录代币的税费请自行核对。</p>}{slippage > 100 && <p className="swap-alert">较高滑点可能导致较差成交价格，请谨慎使用。</p>}<div className="swap-settings-note"><b>授权额度：仅本次数量</b><p>授权对象以确认窗口为准：收费兑换使用已核验的蝴蝶收费合约；包装转换使用 WBNB。BNB 支付不需要代币授权。</p><b>交易有效期：2 分钟</b><p>报价 30 秒过期；确认窗口中的数量不会自动变化。</p></div><button className="swap-primary swap-submit" onClick={() => setSettings(false)}>完成设置</button></Dialog>}
