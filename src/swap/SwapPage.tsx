    ...
    <main className="swap-main">
      ...
      <div className="swap-layout">
        ...
        <section className="swap-workspace" id="swap-form" aria-label="代币兑换">
          ...
          <div className="swap-route-card"><div><span className="swap-route-symbol">↳</span><strong>{quote?.wrap ? 'BNB / WBNB 包装转换' : '你的兑换路径'}</strong><span className="swap-route-status">{quoteBusy ? '报价中' : quote ? stale ? '待刷新' : '已报价' : '等待数量'}</span></div>{quote ? <><div className="swap-path-tokens">{quote.path.map((a, i) => { const token = pathToken(a); return <span key={a}>{i > 0 && <i>→</i>}{token && <TokenIcon token={token} />}<b>{token?.symbol ?? short(a)}</b></span> })}</div><small>区块 #{quote.block.toString()} · {quote.wrap ? '1:1 转换' : `PancakeSwap V2 · ${quote.path.length - 1} 次池内兑换`}</small>{!quote.wrap && <details className="swap-route-comparison"><summary>已比较 {quote.checkedPaths} 条候选路径 · {quote.alternatives?.length} 条有报价</summary><div>{quote.alternatives?.map((r, i) => <div key={r.path.join('-')}><span>{r.path.map(a => pathToken(a)?.symbol ?? short(a)).join(' → ')}{i === 0 && <em>最高输出</em>}</span><b>{displayAmount(r.amountOut, output.decimals)} {output.symbol}</b></div>)}</div><p>相同区块比较输出，未扣网络费；不含 V3 / Infinity，非全市场最优。</p></details>}</> : <p>输入数量后比较直连与常用中间币路径，最多经过 3 个 V2 池。</p>}</div>
        </section>
      </div>
