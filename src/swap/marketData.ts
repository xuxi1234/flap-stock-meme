...
const dexNames: Record<string,string> = { pancakeswap: 'PancakeSwap', thena: 'THENA', uniswap: 'Uniswap', biswap: 'Biswap', apeswap: 'ApeSwap' }
...
export function normalizeMarkets(value: unknown, addresses: string[]): Record<string, TokenMarket> {
  ...
  for (const raw of value) {
    ...
    if (p.chainId !== 'bsc' || !dexNames[p.dexId] || !known.has(address) || !quote || !price || !liquidity || !/^0x[\da-fA-F]{40}$/.test(p.pairAddress ?? '')) continue
