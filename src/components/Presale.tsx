...
export function Presale({ copy, contractAddress = projectConfig.presale.contractAddress, provider, publicClient, walletClient, now = Date.now, onStatusChange, onAccountChange, walletRequest = 0 }: Props) {
  ...
  return <PresaleConsole copy={copy} action={<div className="participation-console" aria-live="polite">
    ...
    {presaleState && presaleState.endTime < BigInt(projectConfig.presale.websiteDeadline) && <p className="status-message is-warning">{copy.interaction.rpcError.includes('链') ? '截止时间延长待链上确认，当前实际截止时间（北京时间）：' : 'Deadline extension awaits on-chain confirmation. Current cutoff (Beijing time): '}{new Intl.DateTimeFormat('zh-CN', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' }).format(new Date(Number(presaleState.endTime) * 1000))}</p>}
