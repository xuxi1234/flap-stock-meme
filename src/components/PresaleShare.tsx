...
export function PresaleShare({ language, account, onConnect }: { onConnect: () => void; language: Language; account: `0x${string}` | null }) {
  ...
  const share = async () => {
    ...
    try { await navigator.share({ title: '蝴蝶股票 | FLAP STOCK', text: projectConfig.presale.websiteOpen ? (zh ? '蝴蝶股票私募：每地址固定 0.05 BNB。请在官网连接钱包，查看链上状态与参与规则。' : '蝴蝶股票 private sale: fixed 0.05 BNB per address. Connect your wallet on the website to check chain status and participation rules.') : (zh ? '蝴蝶股票私募暂未开放，敬请等待官方通知。关注官网与官方社区获取最新动态。' : 'The 蝴蝶股票 private sale is not open. Follow the website and official community for announcements.'), url: shareUrl }) }
  ...
  return <div className="presale-share" aria-label={zh ? '分享私募' : 'Share presale'}>
    ...
    <input aria-label={zh ? '私募分享链接' : 'Presale share link'} placeholder={zh ? '连接钱包后显示含钱包地址的链接' : 'Your wallet share link appears after connecting'} value={shareUrl} readOnly onFocus={event => event.target.select()} />
