...
export function MintPanel({account,busy,run,status,review,connect}:Props){
 ...
 const [target,setTarget]=useState('2'),[days,setDays]=useState('7'),[minimum,setMinimum]=useState('100000'),[shares,setShares]=useState(query.get('mintShares')==='2'?'2':'1')
 useEffect(()=>{
 ...
 {verified&&<>{campaignInput&&<section className="vl-card"><h2>3 Mint 项目已选中</h2><p><code>{campaignInput}</code></p>{campaignInput.toLowerCase()===acceptedMint.campaign.toLowerCase()&&<><p>验收项目已创建，无需重复创建。请按当前验收步骤操作；实际份额以下方链上数据为准。</p><a href={explorer(acceptedMint.transactionHash)} target="_blank" rel="noreferrer">查看项目创建交易</a></>}<button disabled={busy} onClick={()=>{invalidate();setCampaignInput('');setCampaign(null)}}>创建其他 Mint 项目</button></section>}{!campaignInput&&<section className="vl-card"><h2>3 创建 Mint 项目</h2><p>默认资料为验收测试资料。每份 0.01 BNB；买卖税各 3%，税收金库份额 100%，回购间隔 60 秒、单次支出比例 10%。</p><div className="vl-fields">{field('名称',name,setName)}{field('符号',symbol,setSymbol)}{field('目标份额',target,setTarget)}{field('募集天数',days,setDays)}{field('发射最低代币输出（18 位精度）',minimum,setMinimum)}{field('Flap 资料 CID',meta,setMeta)}</div><p>最低输出是发射交易的保护阈值，并非收益承诺；设置过高可能无法发射，届时仍可退款。</p><button disabled={busy} onClick={()=>void run(create)}>检查创建 Mint 项目</button></section>}
