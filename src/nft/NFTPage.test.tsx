...
describe('NFT public payment gates',()=>{
 it('shows actual collection while mint payment stays disabled without deployment',async()=>{render(<NFTPage/>);await screen.findByRole('button',{name:'查看 蓝闪蝶 #0001'});fireEvent.click(screen.getByRole('button',{name:'随机铸造一只 ↗'}));expect(screen.getByRole('button',{name:'尚未开放付款'})).toBeDisabled();expect(screen.queryByText(/免费模拟|体验购买/)).not.toBeInTheDocument();});
