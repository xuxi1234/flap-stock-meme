import {createPublicClient,createWalletClient,http,custom,parseAbi,isAddress,keccak256,zeroAddress,type Address,type Hash,type EIP1193Provider,type WriteContractParameters} from 'viem';
import {bsc} from 'viem/chains';
import deployment from './deployment.json';
import {MINT_WEI,NFT_SUPPLY,TREASURY,type Listing} from './model';
export const nftAbi=parseAbi([
 ...
 'function requestMint() payable returns(uint256)','function claim(uint256,address)','function requests(uint256) view returns(address payer,uint256 tokenId,bool claimed)',
...
export async function validateLive(){
 ...
 const [network,nftCode,marketCode,price,supply,treasury,fee,collection]=await Promise.all([
 ...
 publicClient.readContract({address:d.market,abi:marketAbi,functionName:'FEE_BPS'}),publicClient.readContract({address:d.market,abi:marketAbi,functionName:'collection'})]);
 if(network!==56||!nftCode||!marketCode||keccak256(nftCode)!==d.collectionCodeHash||keccak256(marketCode)!==d.marketCodeHash||price!==MINT_WEI||supply!==BigInt(NFT_SUPPLY)||treasury.toLowerCase()!==TREASURY.toLowerCase()||fee!==0n||collection.toLowerCase()!==d.collection.toLowerCase())throw Error('合约核验失败，已停止付款入口');
 return d;
}
...
export type PendingMint={requestId:bigint;tokenId:number};
export async function readState(account?:Address){
 ...
 if(account){
  ...
  for(let offset=0n;offset<requests;offset+=100n){const requestIds=await publicClient.readContract({address:d.collection,abi:nftAbi,functionName:'payerRequests',args:[account,offset,100n]});
   for(const r of await chunks(requestIds,async requestId=>({requestId,data:await publicClient.readContract({address:d.collection,abi:nftAbi,functionName:'requests',args:[requestId]})})))if(!r.data[2]&&r.data[0].toLowerCase()===account.toLowerCase())pending.push({requestId:r.requestId,tokenId:Number(r.data[1])});}
 }
 return{total:Number(total),reserved:Number(reserved),owned,pending,listings:rows.filter(x=>x.active||(account&&x.seller.toLowerCase()===account.toLowerCase()&&x.price>0n))};
}
...
export async function transact(action:'mint'|'claim'|'approve'|'list'|'cancel'|'buy',account:Address,args:{id?:number;requestId?:bigint;price?:bigint},onHash:(hash:Hash)=>void){
 ...
 let request;
 if(action==='mint')request=await publicClient.simulateContract({address:d.collection,abi:nftAbi,functionName:'requestMint',account,value:MINT_WEI});
