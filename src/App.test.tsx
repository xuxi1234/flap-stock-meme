...
describe('simplified bilingual brand page', () => {
  ...
  it('opens the trading workspace and preserves the market link', async () => {
    ...
    fireEvent.click(screen.getByRole('link', { name: '关于', exact: true }))
    expect(screen.getByRole('heading', { name: '交易清楚，分配透明。' })).toBeInTheDocument()
    expect(new URLSearchParams(window.location.search).get('section')).toBe('about')
    expect(screen.queryByRole('region', { name: '代币兑换' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('link', { name: '探索', exact: true }))
    expect(screen.getByRole('tab', { name: '代币排行' })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('link', { name: '交易', exact: true }))
    expect(screen.getByRole('region', { name: '代币兑换' })).toBeInTheDocument()
    window.history.replaceState(null, '', '/')
  })
})
