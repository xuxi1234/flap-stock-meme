...
export default function CommunityAirdrop(){
 ...
 useEffect(()=>{if(!provider)return;const changed=()=>{revision.current++;setWallet('');setPlan(null);setBalance('—')};provider.provider.on('accountsChanged',changed);provider.provider.on('chainChanged',changed);return()=>{provider.provider.removeListener('accountsChanged',changed);provider.provider.removeListener('chainChanged',changed)}},[provider])
 ...
 async function review(){const version=++revision.current;setBusy(true);setError('');setPlan(null);try{if(decimals===undefined)throw Error('请等待代币精度读取成功。');const rows=validateRecipients(list,wallet);for(let i=0;i<rows.length;i+=10){const codes=await Promise.all(rows.slice(i,i+10).map(r=>rpc.getCode({address:r.address as Address})));if(codes.some(c=>c&&c!=='0x'))throw Error('名单含合约地址，请更换后重新检查。')}if(version!==revision.current)return;setPlan({token:TOKEN,decimals,rows,mode:'community200'})}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
 ...
 <section className="ad-card" id="ca-start"><p className="ad-eyebrow">01 / SELECT RECIPIENTS</p><h2>这次，送给谁？</h2><div className="ca-tabs"><button disabled={busy} aria-pressed={mode==='platform'} onClick={()=>{setMode('platform');setList('');setPlan(null)}}>平台链上名单</button><button disabled={busy} aria-pressed={mode==='custom'} onClick={()=>{setMode('custom');setList('');setPlan(null);setSource('自行导入')}}>自己的 200 个地址</button></div>
 {mode==='platform'?<><p>筛选近期在 BNB Chain 主动发起交易的钱包，过滤重复、合约和禁止接收地址。名单来自公开链上活动，不代表这些钱包已订阅空投，也不保证触达真人。</p><button className="ad-export" disabled={busy} onClick={()=>void platform()}>{busy?'正在检查…':'获取 200 个地址'}</button><small>同一时间获取的名单可能重叠；链上防重仅针对同一发送钱包和同一批次。不会生成无人控制的随机地址。</small></>:<><p>粘贴 200 个地址，或上传只有地址列的 CSV / TXT。每个地址固定发送 1 枚。</p><input disabled={busy} aria-label="上传地址名单" type="file" accept=".csv,.txt" onChange={async e=>{const f=e.target.files?.[0];if(!f)return;if(f.size>100000){setError('文件不能超过 100 KB');return}setList(await f.text());setPlan(null)}}/></>}
 <textarea disabled={busy} aria-label="接收地址" placeholder="收款名单会显示在这里，每行一个地址" readOnly={mode==='platform'} value={list} onChange={e=>{setList(e.target.value);setPlan(null)}}/>
