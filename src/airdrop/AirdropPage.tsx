...
const number = (value: string) => { const [a, b] = value.split('.'); return Number(a).toLocaleString('en-US', { maximumFractionDigits: 0 }) + (b ? '.' + b : '') }
...
export default function AirdropPage() {
  ...
  async function connect(detail: WalletProviderDetail) {
    ...
    try {
      ...
      setWallet(accounts[0]); setWalletMenu(false)
  ...
  function generate() {
    ...
    try {
      ...
      if (mode === 'presale') {
        ...
        rows = recipients.map(address => ({ address, amount: amountText(units(perAddress, decimals), decimals), group: mode === 'random' ? '随机模拟地址' : '导入地址' }))
      }
  ...
  function exportPlan() {
    if (!plan) return
    download('蝴蝶股票_发放预览.csv', csvText([['序号', '地址或待提供名额', '代币数量', '分组', '代币合约', '链ID', '精度', '状态'], ...plan.rows.map((r, i) => [i + 1, r.address, r.amount, r.group, plan.token, '56', plan.decimals, '仅预览，未发送'])]))
  }
