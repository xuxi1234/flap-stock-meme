...
/// @dev Chainlink VRF v2.5 subscription ABI; pinned coordinator, LINK subscription billing.
...
contract ButterflyVRF {
 ...
 constructor(address coordinator_,bytes32 keyHash_,uint256 subscriptionId_,uint16 confirmations_,uint32 gasLimit_) {
  require(coordinator_.code.length>0 && keyHash_!=bytes32(0) && subscriptionId_!=0 && confirmations_>=3 && gasLimit_>=200000,'Invalid VRF configuration');
 ...
 function request() external returns(uint256 requestId) {
  ...
  requestId=coordinator.requestRandomWords(IVRFCoordinatorV2Plus.RandomWordsRequest({keyHash:keyHash,subId:subscriptionId,requestConfirmations:confirmations,callbackGasLimit:callbackGasLimit,numWords:1,extraArgs:abi.encodeWithSelector(bytes4(keccak256('VRF ExtraArgsV1')),false)}));
