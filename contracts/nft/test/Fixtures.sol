...
contract RejectNFT {}
interface CollectionRequests {function requestMint() external payable returns(uint256);}
contract BatchRequester {
 function reserve(address collection,uint256 count) external payable {
  for(uint256 i;i<count;++i) CollectionRequests(collection).requestMint{value:0.01 ether}();
 }
}
